...
|flushall|清空数据库|
|shutdown|保证服务器正常关闭且不丢失任何数据，意义...也不大|
...
#### AOF
aof 默认是不开启的，弥补了rdb的不足（数据不一致性），所以它采用日志的形式来记录每个写操作，并追加到文件中。Redis 重启的会根据日志文件的内容将写指令从前到后执行一次以完成数据的恢复工作。
...
**aof优缺点**  
1. 数据的完整性和一直性高
2. AOF记录的内容多，文件会越来越大，数据恢复也会越来越慢
...
**pipeline 管道模式**可以在服务端未响应时，客户端可以继续向服务端发送请求，并最终一次性读取所有服务端的响应  
  ***Pipeline*** 是redis提高吞吐量的机制，适用于多key读写场景，会占用大量内存（用pipeline方式打包发送命令，redis必须
  ...
  pipeline 期间独占 **连接**,此期间将不能进行非“管道”类型的其他操作，直到 pipeline 关闭；
